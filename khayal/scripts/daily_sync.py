...
def fetch_trending(
    ...
    """Fetch trending items from TMDB (requires network)."""
    try:
        import httpx
    ...
    return [build_media_record(item, media_type) for item in data.get("results", [])]
...
def run_sync() -> None:
    ...
    for media_type in ("movie", "tv"):
        records = fetch_trending(media_type, "day", tmdb_key)
